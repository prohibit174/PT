						...
						<ul class="sets-list ajax-content">
							...
							<c:forEach var="product" items="${list}">
								<li itemscope="" itemtype="http://schema.org/ImageObject"
									class="same-height"><a
									href="product_detail${pageMaker.makeQuery(pageMaker.cri.page) }&p_num=${product.p_num }"
									itemprop="url" style="color: DarkSlateGray";> <picture
											class="img"> <c:if test="${product.p_img!=null}">
											<c:set var="head"
											...
											<c:choose>
												<c:when
													...
													<img src="/resource/upload/${head }_small.${pattern}"
														alt="img /">
												</c:when>
												<c:otherwise>
		...
		<div class="text-center">
			<ul class="pagination">
				...
				<c:if test="${pageMaker.prev}">
					<li><a href="product_listPage?page=${pageMaker.startPage - 1}">&laquo;</a></li>
				</c:if>
				...
				<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
					<li><a href="product_listPage?page=${pageMaker.endPage +1}">&raquo;</a></li>
				</c:if>
	...
	<script>
		...
		$(".pagination li a").on(
				...
				function(event) {
					...
					jobForm.attr("action", "/product/product_listPage").attr(
							"method", "get");
					jobForm.submit();
				});
	</script>
