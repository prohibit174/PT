  ...
  <div class="section-links" style="margin-top:0px; padding-top:0px;
		font-size:16px; float: right; margin-right:70px;">
				<ul>
					...
					<li><a href="/acceptDeclareList">ACCEPT LIST</a></li>
					<li>REJECT LIST</li>
				</ul>
	</div>
	...
	<div class="heading">
		...
		<ul class="table2 ajax-content">
			...
			<c:forEach items="${rejectList }" var="rejectList">
					<div class="th col-first"> ${rejectList.u_id }</div>
					<div class="th col1">${rejectList.d_reported }</div>
					<div class="th col2">${rejectList.d_small }</div>
					<form id="fileView${rejectList.d_num }" name="fileView${rejectList.d_num }" method="post" 
 						action="/fileview?d_num=${rejectList.d_num }" target="popup_window">	
 					<a onclick = "fileView(${rejectList.d_num });">
					<div class="th col2">${rejectList.d_file }</div>
					</a>
					<div class="th col3"></div>
					</form>
					
					
					<form id="content${rejectList.d_num }" name="content${rejectList.d_num }" method="post" 
 						action="/detailContent?d_num=${rejectList.d_num }" target="popup_window">
 					<a onclick = "content(${rejectList.d_num });">
